...
//alert(days+"-"+hours+"-"+minutes+"-"+seconds);
	...
	{
		$("#invoicebutton").text("Facture générée");
	}
</script>
'; ?>
				...
				<p>Date de livraison</p>
					...
					<?php else: ?>
						...
						<?php elseif ($this->_tpl_vars['aoparticipation'][0]['status'] == 'closed_client'): ?>
							<p class="num-large less24" style="font-size:24px">Fermé</p>
						<?php else: ?>
...
" onClick="updateinvoicetext();" id="invoicebutton" class="btn btn-small" rel="tooltip" data-html="true" data-original-title="<i class='icon-exclamation-sign icon-white'></i> Attention ! En demandant une facture, vous consid&eacute;rez la commande d&eacute;finitivement clotûr&eacute;e." data-placement="bottom">
								...
								<?php if ($this->_tpl_vars['aoparticipation'][0]['status'] != 'closed_client_temp' && $this->_tpl_vars['aoparticipation'][0]['status'] != 'published' && $this->_tpl_vars['aoparticipation'][0]['status'] != 'closed' && $this->_tpl_vars['aoparticipation'][0]['status'] != 'closed_client'): ?>
									<a class="btn btn-small" rel="tooltip" id="refbutton" data-original-title="Refuser définitivement la livraison" data-placement="bottom" data-target="#refuse-ajax" data-toggle="modal" role="button"><i class="icon-thumbs-down"></i> Refuser la livraison</a>
								<?php endif; ?>
						...
						<?php if ($this->_tpl_vars['aoparticipation'][0]['paid_status'] == 'paid' && $this->_tpl_vars['aoparticipation'][0]['royalty'] == 'YES' && $this->_tpl_vars['aoparticipation'][0]['status'] != 'disapprove_client' && $this->_tpl_vars['aoparticipation'][0]['status'] != 'closed_client_temp' && $this->_tpl_vars['aoparticipation'][0]['status'] != 'closed' && $this->_tpl_vars['aoparticipation'][0]['status'] != 'closed_client'): ?>   
							<div class="muted">Sans action de votre part, la livraison sera considérée comme validée sous 24h</div>
						<?php endif; ?>
					<?php endif; ?>
				</div>
			</section>
