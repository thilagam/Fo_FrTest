...
class FtveditoController extends Ep_Controller_Action
{
	public function init() 
	{
        ...
        if(isset($_COOKIE['ftvedito']))
        {  // echo $_COOKIE['editouser']; echo base64_decode($_COOKIE['editopass']);
            $obj = new Ep_Ftv_FtvContacts();
            $details = $obj->getContactDetails($_COOKIE['editouser']);
            $res= $obj->checkFtvLogin($_COOKIE['editouser'],base64_decode($_COOKIE['editopass']), $via='direct', $details[0]['ftvtype'], 'edito');
            if($res!="NO")
            {
                $this->ftvcontacts->ftvId =$res[0]['identifier'];
                $this->ftvcontacts->ftvmailId =$res[0]['email_id'];
            }
            //$this->_redirect("/ftvedito/index");
           // header('Location: http://mmm-new.edit-place.com/ftvedito/index');
           // $this->indexAction();
		...
		if($this->ftvcontacts->ftvId != "")
		{
            ...
            if($requestsdetail != 'NO')
            {
                foreach ($requestsdetail as $key => $value)
                {
                   ...
                   $containvalue = implode("<br/>",$finalcontains);
            ...
            $this->render("ftv_index");
		}
		else
		{
			$this->render("ftv_login");
		}
	}
    ...
    public function newrequestAction()
    {
        ...
        {  // echo $this->ftvcontacts->ftvId; exit;
            $ftvrequest_obj = new Ep_Ftv_FtvRequests();
            $request_params = $this->_request->getParams();
            $othercontacts =  implode(",",$request_params["othercontacts"]);
            $quand = implode(",",$request_params["quand"]);
            ...
            try
            {
                ...
                if($ftvrequest_obj->insert())
                {
                    ...
                    foreach($othercontacts as $values)
                    {
                        ...
                        $automail->sendFtvContactsPersonalEmail($values,112,$parameters); // to client contact
                    }
    ...
    public function uploadfilesAction()
    {
        ...
        {       //print_r($doc_params); print_r($_FILES);  exit;
            /*if($this->ftvcontacts->ftvId != '')
                $this->_redirect("/ftvedito/index");*/
    ...
    public function savecommentsAction()
    {
        ...
        if($_SERVER['HTTP_X_REQUESTED_WITH']=='XMLHttpRequest')
        {
            ...
            foreach($othercontacts as $values)
            {
                ...
                $automail->sendFtvContactsPersonalEmail($values,116,$parameters); // to client contact
	...
	public function uservalidationajaxAction()
	{
        ...
        setcookie("ftvedito", "ftvedito", $expire);
        setcookie("editouser", $emailcheck_params_login['login_name'], $expire);
        setcookie("editopass", base64_encode($emailcheck_params_login['login_password']), $expire);
		$obj = new Ep_Ftv_FtvContacts();
        $details = $obj->getContactDetails($emailcheck_params_login['login_name']);
		$res= $obj->checkFtvLogin($emailcheck_params_login['login_name'],$emailcheck_params_login['login_password'], $via='direct', $details[0]['ftvtype'], 'edito');
		//update last visit
		if($res!="NO")
		{   
            //print_r($res);
	...
	public function logoutAction()
	{
        ...
        $this->ftvcontact = Zend_Registry::get('EP_ftvcontacts');
        setcookie("ftvedito", "ftvedito", time()-360);
        setcookie("editouser", "", time()-360);
        setcookie("editopass", "", time()-360);
        /*echo $timeLeftTillSessionExpires = $_SESSION['__ZF']['EP_ftvcontacts']['ENT'] - time();
        var_dump($this->ftvcontact);exit;*/
		Zend_Session::destroy('EP_ftvcontacts');
		$this->_redirect("/ftvedito/index");
	}
	// add by naseer //
    /*public function loadftvrequestsAction()
    {
        ...
        if($this->ftvcontacts->ftvId != "")
        {
            ...
            $data =  $requestsdetail;
            //echo "<pre>";print_r($requestsdetail);exit;
            //$this->_view->requestsdetail=ftvmailIdftvmailId;

            //$this->render("ftv_index");
        }
        $aColumns = array('client' , 'created_at', 'request_object', 'download','duration','modify_contains', 'modify_broadcast', 'modify_broadcast',
        'demand', 'comments', 'add_comments', 'action' );

        //$data = array(
        //    array('client' => 'naseer', 'created_at'  => 'naseer', 'request_object'  => 'naseer', 'download' => 'naseer','duration' => 'naseer','modify_contains' => 'naseer', 'modify_broadcast' => 'naseer', 'modify_broadcast' => 'naseer',
        //        'demand' => 'naseer', 'comments' => 'naseer', 'add_comments' => 'naseer', 'action'  => 'naseer')
        //);
    ...
    public function loadftvrequestsAction()
    {
        ...
        $aColumns = array( 'first_name', 'created_at', 'request_object','download','duration','modify_contains',
            'modify_broadcast','demand','status','comments','add_comments','actions');
        /* * Paging	 */
        $sLimit = "";
                    ...
                    for ( $j=0 ; $j<=count($aColumns) ; $j++ )
                    {
                        ...
                        //else
                        {
                            ...
                            elseif($aColumns[$j] == 'request_object')
                                $row[] = '<a data-target="#chaineeditrequest" data-refresh="true"  data-toggle="modal" href="/ftvedito/duplicaterequest?request_id='.$requestsdetail[$i]["identifier"].'&edit=yes" class="hint--bottom hint--info" data-placement="right" data-original-title="Object" data-html="true"
                            ...
                            elseif($aColumns[$j] == 'download') {
                                $download = '<a class="hint--bottom hint--info"  data-target="#fileupload" rel="tooltip" data-original-title="Ajouter un fichier" data-refresh="true"  data-toggle="modal" data-hint="upload file"  role="button"   href="/ftvchaine/uploadfiles?request_id=' . $requestsdetail[$i]["identifier"] . '&ftvtype=ftvedito&view=yes">
                            ...
                            elseif($aColumns[$j] == 'actions') {
                                $action = '<a  data-target="#newrequest" data-refresh="true"  data-toggle="modal" rel="tooltip" data-original-title="Dupliquer la demande" role="button"   href="/ftvedito/duplicaterequest?request_id=' . $requestsdetail[$i]['identifier'] . '&edit=no"><img class="img-responsive pull-left" src="/FO/images/imageB3/duplicate_icon.jpg" height="20" width="20" /></a>';
                                if ($requestsdetail[$i]['assigned_to'] == '')
                                    $action .= '<a  data-target="#newrequest" data-refresh="true"  data-toggle="modal" rel="tooltip" data-original-title="Modifier la demande" role="button"   href="/ftvedito/duplicaterequest?request_id=' . $requestsdetail[$i]['identifier'] . '&edit=yes"><img class="img-responsive pull-left" src="/FO/images/imageB3/edit_icon.png" height="20" width="20" /></a>';
                                $row[] = $action;
                            }
    ...
    //function to insert new broadcast into xml file//
    public function addNewBroadcastAction(){
        $new_broadcast = $_REQUEST['new_broadcast'];
        $count =  $_REQUEST['count'];
        //open xml file
        $xml_file_path = DATA_PATH."translation/EP_BROADCASTS.xml";
        $xml = simplexml_load_file($xml_file_path) or die("Error: Cannot create object");

        $sxe = new SimpleXMLElement($xml->asXML());
        $itemsNode = $sxe->EP_BROADCASTS[0]->fr[0]->EP_BROADCASTS[0];
        $itemsNode->addChild("em".$count, $new_broadcast);

        //echo "<pre>";print_r($sxe);//testing content
        $sxe->asXML($xml_file_path); //saving xml file
        $source = DATA_PATH."translation/EP_BROADCASTS.xml";
        $destination = DATA_PATH_BO."translation/EP_BROADCASTS.xml";
        echo $source."\n".$destination;
    ...
    public function checkExistanceAction(){
        $broadcast_array=$this->_arrayDb->loadArrayv2("EP_BROADCASTS", $this->_lang);
        if(in_array($_REQUEST['new_broadcast'],$broadcast_array) )
            echo 1;
        else
            echo 0;
        unset($_REQUEST);
    ...
    /*end of added by naseer on 02-09-2015*/
